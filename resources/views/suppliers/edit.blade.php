...
@section('content')
<x-page-header title="تعديل: {{ $supplier->name }}">
    <a href="{{ route('suppliers.index') }}" class="text-sm text-slate-500 hover:text-slate-700">← العودة</a>
</x-page-header>
<div class="max-w-lg">
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        <form method="POST" action="{{ route('suppliers.update', $supplier) }}" class="space-y-4">
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('suppliers.index') }}" class="text-slate-600 text-sm px-5 py-2 rounded-lg border border-slate-300 hover:bg-slate-50 transition-colors">إلغاء</a>
            </div>
        </form>
    </div>
</div>
