...
@section('content')
    <x-page-header title="إضافة منتج جديد"><a href="{{ route('products.index') }}"
        ...
        <form method="POST" action="{{ route('products.store') }}">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input name="price" type="number" step="0.01" value="{{ old('price', 0) }}"
                        class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('price')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-6 flex justify-end gap-3">
                ...
                <button type="submit"
                    class="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 transition-colors">حفظ
                    المنتج</button>
            </div>
        </form>
    </div>
