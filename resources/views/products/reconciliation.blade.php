...
@section('content')
    ...
    @php $totalRows = collect($similarities)->sum('count'); @endphp
    <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
        <div class="flex items-start gap-3">
            ...
            <div>
                <h3 class="text-amber-800 font-semibold mb-1">تم اكتشاف {{ count($similarities) }} منتج متشابه</h3>
                <p class="text-amber-700 text-sm">من أصل {{ $totalRows }} صفًا مكررًا. يرجى مراجعة كل منتج واختيار الاسم
                    الصحيح من القائمة أو إنشاء اسم جديد.</p>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('products.reconciliation.store') }}" id="reconciliationForm">
        ...
        <div class="space-y-4">
            @forelse($similarities as $index => $data)
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm">
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                <span class="text-xs text-slate-500 block mb-1">
                                    الصف {{ $data['row_number'] ?? '-' }}
                                    @if (!empty($data['count']) && $data['count'] > 1)
                                        - عدد التكرارات {{ $data['count'] }}
                                    @endif
                                </span>
                            ...
                            <div class="flex items-center gap-2">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="choices[{{ $index }}][create_new]" value="1"
                                        class="w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500"
                                        onchange="toggleCreateNew(this, {{ $index }})">
                    ...
                    <div class="p-4">
                        <input type="hidden" name="choices[{{ $index }}][original]"
                            value="{{ $data['original'] }}">
                        ...
                        <div class="mt-3 pt-3 border-t border-slate-200">
                            <div class="flex gap-2">
                                <input type="text" placeholder="ابحث عن منتج آخر..."
                        ...
                        <div id="create-new-{{ $index }}" class="mt-3 pt-3 border-t border-slate-200 hidden">
                            ...
                            <input type="text" name="choices[{{ $index }}][new_name]"
                                value="{{ $data['original'] }}"
        ...
        <div class="mt-6 flex justify-end gap-3">
            <a href="{{ route('imports.index') }}"
                class="px-4 py-2 border border-slate-300 rounded-lg text-slate-700 hover:bg-slate-50 transition-colors text-sm">
                إلغاء
            </a>
            <button type="submit"
            ...
            searchTimeouts[index] = setTimeout(() => {
                ...
                `).join('');
                            ...
                            resultsDiv.innerHTML =
                                '<div class="text-slate-500 text-sm py-2">لا توجد نتائج</div>';
                            resultsDiv.classList.remove('hidden');
                        }
                    })
                    .catch(error => {
